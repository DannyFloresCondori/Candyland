...
export default function SoldOrdersPage() {
  ...
  // Filtrar solo los pedidos vendidos
  ...
  // Agrupar por día
  const groupedOrders: Record<string, Order[]> = soldOrders.reduce((acc, order) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-600">Ver todas las órdenes que han sido vendidas, agrupadas por día</p>
      </div>
                      ...
                      <TableRow key={order.id}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex gap-2 justify-end">
                            <Button variant="default" onClick={() => setSelectedOrder(order)}>
                              Ver Detalle
                            </Button>
                            <Button 
                              variant="outline" 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="w-full max-w-2xl rounded-lg bg-white p-6">
            ...
            <div className="mt-6 flex justify-end gap-2">
              <Button 
                variant="outline" 
                onClick={() => generateOrderReport(selectedOrder.id, 'order')}
                disabled={isGenerating}
              >
                <FileText className="h-4 w-4 mr-1" />
                Generar PDF
